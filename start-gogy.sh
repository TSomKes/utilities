...
then
    ...
    tmux new-session -s "$sessionName" -n webservice -d
    ...
    # U-R pane starts ready to run local tests (w/o carriage return)
    #tmux send-keys -t "$sessionName":1.1 'python test/webserviceAPI.py local'
    ...
    tmux send-keys -t "$sessionName":1.2 'cd litapp-source/www/' C-m
    tmux send-keys -t "$sessionName":1.2 'python3 -m http.server 8050' C-m
    ...
    tmux send-keys -t "$sessionName":2.1 'git fetch' C-m
    tmux send-keys -t "$sessionName":2.1 'git statz' C-m
